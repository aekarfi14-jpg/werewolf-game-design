...
import firstOutImg from '@/assets/images/memes/first-out.jpg';
...
const memeImages = { wolvesWin: wolvesWinImg, villageWin: villageWinImg, firstOut: firstOutImg };
...
function App() {
  ...
  const [thiefHasStolen, setThiefHasStolen] = useState(false);
  const [firstEliminationDone, setFirstEliminationDone] = useState(false);
  const [showFirstEliminationImg, setShowFirstEliminationImg] = useState(false);
  ...
  function beginGame() {
    ...
    setThiefHasStolen(false);
    setFirstEliminationDone(false);
    setShowFirstEliminationImg(false);
    setPhase('reveal');
  }
  ...
  function submitVote(targetIndex: number) {
    if (targetIndex >= 0) {
      ...
      if (!firstEliminationDone) {
        setFirstEliminationDone(true);
        setShowFirstEliminationImg(true);
      }
  ...
  function revivePlayer(index: number) {
    setAlivePlayers((current) => [...current, index].sort((a, b) => a - b));
  }

  function newRound() { setRound((c) => c + 1); setPhase('reveal'); setShowFirstEliminationImg(false); }

  function goToVote() { setShowFirstEliminationImg(false); setPhase('vote'); }
  ...
  return <main className="app-shell dashboard"><header className="topbar"><button className="brand-mark compact" onClick={() => setPhase('home')}><Moon size={18} /> ليلة الذئاب</button><div className="top-actions"><button className="icon-button" onClick={goToLog} title="السجل"><BookOpen size={18} /></button><button className={`sound-toggle ${soundOn ? 'active' : ''}`} onClick={() => setSoundOn((c) => !c)}>{soundOn ? <Volume2 size={16} /> : <VolumeX size={16} />} {soundOn ? 'الأصوات' : 'كتم'}</button><button className={`sound-toggle ${musicOn ? 'active' : ''}`} onClick={() => setMusicOn((c) => !c)}><Music size={16} /> {musicOn ? 'موسيقى' : 'إيقاف'}</button><span className="round-pill">الجولة {round}</span></div></header>
    ...
    {phase === 'night' && <Night players={players} roles={roles} alivePlayers={alivePlayers} enabled={enabled} playSound={playSound} onFinish={finishNight} witchPotionsLeft={witchPotionsLeft} setWitchPotionsLeft={setWitchPotionsLeft} nightCount={nightCount} eliminatePlayer={eliminatePlayer} guardianLastTarget={guardianLastTarget} setGuardianLastTarget={setGuardianLastTarget} setRoles={setRoles} revivePlayer={revivePlayer} thiefHasStolen={thiefHasStolen} setThiefHasStolen={setThiefHasStolen} />}
    {phase === 'vote' && <Vote players={players} roles={roles} alivePlayers={alivePlayers} onSubmit={submitVote} />}
    {phase === 'result' && <Result logs={logs} roles={roles} alivePlayers={alivePlayers} onVote={goToVote} onNewRound={newRound} onLog={goToLog} showFirstEliminationImg={showFirstEliminationImg} />}
    {phase === 'log' && <LogView logs={logs} onBack={backFromLog} roles={roles} players={players} showAllCards={showAllCards} setShowAllCards={setShowAllCards} alivePlayers={alivePlayers} />}
...
function Night({ players, roles, alivePlayers, enabled, playSound, onFinish, witchPotionsLeft, setWitchPotionsLeft, nightCount, eliminatePlayer, guardianLastTarget, setGuardianLastTarget, setRoles, revivePlayer, thiefHasStolen, setThiefHasStolen }: { players: string[]; roles: RoleKey[]; alivePlayers: number[]; enabled: Record<RoleKey, boolean>; playSound: (type: SoundType) => void; onFinish: (results: LogEntry[]) => void; witchPotionsLeft: { kill: boolean; protect: boolean }; setWitchPotionsLeft: (v: { kill: boolean; protect: boolean }) => void; nightCount: number; eliminatePlayer: (index: number) => boolean; guardianLastTarget: number | null; setGuardianLastTarget: (v: number | null) => void; setRoles: (r: RoleKey[]) => void; revivePlayer: (index: number) => void; thiefHasStolen: boolean; setThiefHasStolen: (v: boolean) => void }) {
  const steps: { role: RoleKey; title: string; sound: SoundType }[] = [
    ...
    ...(enabled.thief && !thiefHasStolen ? [{ role: 'thief' as RoleKey, title: 'السارق يستيقظ', sound: 'night' as SoundType }] : []),
  ];
  ...
  function next() {
    if (isLast) {
      const killedThisNight: number[] = [];
      let wolfKilledName: string | null = null;
      let witchKilledName: string | null = null;
      let revivedName: string | null = null;
      let theftTargetName: string | null = null;
      ...
      if (wolfTarget !== null) {
        ...
        if (!protectedByWitch && !protectedByGuardian) {
          ...
          wolfKilledName = players[wolfTarget];
        }
      }

      if (witchPotionsLeft.kill && witchKillTarget !== null) {
        killedThisNight.push(witchKillTarget);
        witchKilledName = players[witchKillTarget];
      }

      if (alphaReviveChoice !== null && eliminatedWolves.length > 0) {
        revivedName = players[alphaReviveChoice];
      }

      let theftHappened = false;
      if (thiefTarget !== null && !thiefHasStolen) {
        const thiefIndex = alivePlayers.find((i) => roles[i] === 'thief');
        if (thiefIndex !== undefined && thiefIndex !== thiefTarget) {
          theftHappened = true;
          theftTargetName = players[thiefTarget];
        }
      }

      const results: LogEntry[] = [];

      const allDeadNames: string[] = [];
      if (wolfKilledName) allDeadNames.push(wolfKilledName);
      if (witchKilledName && !allDeadNames.includes(witchKilledName)) allDeadNames.push(witchKilledName);

      if (allDeadNames.length > 0) {
        if (allDeadNames.length === 1) {
          results.push({ title: 'ضحايا الليلة', text: `الذيابة هجموا الليلة، والضحية هي ${allDeadNames[0]}`, tone: 'red' });
        } else {
          results.push({ title: 'ضحايا الليلة', text: `الليلة طاح جماعة: ${allDeadNames.join(' و ')}`, tone: 'red' });
        }
      }

      if (revivedName) {
        results.push({ title: 'رجعة للحياة', text: `قائد الذيابة رجع ${revivedName} للحياة`, tone: 'dark' });
      }

      if (theftTargetName) {
        results.push({ title: 'سرقة دور', text: `الجاسوس سرق دور ${theftTargetName}، وولا قروي عادي`, tone: 'gold' });
      }

      if (results.length === 0) {
        results.push({ title: 'ليلة هادئة', text: 'الليلة دازت بلا ضحايا', tone: 'dark' });
      }
      ...
      if (revivedName !== null && alphaReviveChoice !== null) {
        revivePlayer(alphaReviveChoice);
      }

      if (theftHappened && thiefTarget !== null) {
        const thiefIndex = alivePlayers.find((i) => roles[i] === 'thief');
        if (thiefIndex !== undefined) {
          const newRoles = [...roles];
          const stolenRole = newRoles[thiefTarget];
          newRoles[thiefTarget] = 'villager';
          newRoles[thiefIndex] = stolenRole;
          setRoles(newRoles);
          setThiefHasStolen(true);
        }
      }
  ...
  return <section className="game-page night-page"><div className="night-sky"><div className="stars">✦　·　✧　　·　✦</div><Moon size={72} strokeWidth={1} /><div className="eyebrow">الليلة {nightCount} · خطوة {step + 1} من {steps.length}</div><h2>{current.title}</h2></div>
    ...
    {current.role === 'thief' && <div className="night-action"><p className="night-instruction">السارق يستهدف لاعباً لسرقة دوره.</p><div className="night-players">{alivePlayers.filter((i) => roles[i] !== 'thief').map((gi) => <button key={gi} className={`night-player ${thiefTarget === gi ? 'selected' : ''}`} onClick={() => setThiefTarget(gi)}><span>{gi + 1}</span>{players[gi]}{thiefTarget === gi && <Check size={15} />}</button>)}</div></div>}
...
function Result({ logs, roles, alivePlayers, onVote, onNewRound, onLog, showFirstEliminationImg }: { logs: LogEntry[][]; roles: RoleKey[]; alivePlayers: number[]; onVote: () => void; onNewRound: () => void; onLog: () => void; showFirstEliminationImg: boolean }) {
  ...
  return <section className="game-page result-page">{showFirstEliminationImg && <div className="result-meme"><img src={memeImages.firstOut} alt="أول ضحية" loading="lazy" /><div className="result-meme-overlay" /><span className="result-meme-caption">أول واحد يطيح! ما كانش يستاهل</span></div>}<div className="result-meme"><img src={memeImg} alt={memeCaption} loading="lazy" /><div className="result-meme-overlay" /><span className="result-meme-caption">{memeCaption}</span></div><div className="result-icon"><Flame size={30} /></div><div className="eyebrow">انتهت المرحلة</div><h2>الصباح يحمل خبراً جديداً</h2><p className="result-lead">{latest[latest.length - 1]?.text ?? 'القرية تستيقظ ببطء.'}</p><div className="story-stack">{latest.map((item, i) => <div className={`story-item ${item.tone}`} key={i}><span>{item.tone === 'green' ? <Shield size={17} /> : item.tone === 'red' ? <Skull size={17} /> : item.tone === 'gold' ? <Sparkles size={17} /> : <Moon size={17} />}</span><div><b>{item.title}</b><p>{item.text}</p></div></div>)}</div><div className="result-actions"><button className="secondary-button" onClick={onLog}><BookOpen size={16} /> اذهب للسجل يا حكم</button><button className="secondary-button" onClick={onVote}>ابدأ التصويت</button><button className="primary-button" onClick={onNewRound}><RotateCcw size={16} /> الجولة الجديدة</button></div></section>;
}

function LogView({ logs, onBack, roles, players, showAllCards, setShowAllCards, alivePlayers }: { logs: LogEntry[][]; onBack: () => void; roles: RoleKey[]; players: string[]; showAllCards: boolean; setShowAllCards: (v: boolean) => void; alivePlayers: number[] }) {
